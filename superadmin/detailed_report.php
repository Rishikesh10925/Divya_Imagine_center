    ...
    <table class="data-table">
        <thead>
            ...
            <?php elseif ($view === 'tests'): ?>
                 <tr><th>Test Category</th><th>Sub-Test</th><th>Count</th><th>Total Revenue</th><th>Total Discount</th></tr>
            <?php endif; ?>
        </thead>
        <tbody>
            <?php
            $sql = '';
            if ($view === 'bills') {
                $sql = "SELECT b.id, p.name as patient_name, b.created_at, u.username as receptionist_name, rd.doctor_name, b.discount, b.discount_by, b.net_amount FROM bills b JOIN patients p ON b.patient_id = p.id JOIN users u ON b.receptionist_id = u.id LEFT JOIN referral_doctors rd ON b.referral_doctor_id = rd.id WHERE $where_sql ORDER BY b.id DESC";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param($types, ...$params);
                $stmt->execute();
                $result = $stmt->get_result();
                while($row = $result->fetch_assoc()) {
                    echo "<tr><td>{$row['id']}</td><td>".htmlspecialchars($row['patient_name'])."</td><td>".date('d-m-Y', strtotime($row['created_at']))."</td><td>".htmlspecialchars($row['receptionist_name'])."</td><td>".($row['doctor_name'] ? 'Dr. '.htmlspecialchars($row['doctor_name']) : 'Self')."</td><td>".number_format($row['discount'], 2)."</td><td>".htmlspecialchars($row['discount_by'])."</td><td>".number_format($row['net_amount'], 2)."</td></tr>";
                }
